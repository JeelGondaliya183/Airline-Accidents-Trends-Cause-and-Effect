...
function main(error, worldLocations) {
	...
	fetch("http://localhost:5000/getHeatMapData",{method:"GET"}).then(response=>{
...
function formGraph(dests){
    ...
    dests.forEach(d => { d.coords = projection([d.Longitude, d.Latitude]); })
    
	
	//d3 topojson to create features referred by [26]
    var countries = topojson.feature(global_world, global_world.objects.countries).features;
...
fetch("http://localhost:5000/getEventCountData",{method:"GET"}).then(response =>{		
	...
	keys.forEach(key => {
		if(JSON.parse(lineData[key]).length > 1)
			year.append("option").attr("value",key).text(key);
		else if(key == "2019")
		{
			var futureArray = [];
			...
			year.append("option").attr("value","Future").text("Future");			//appending Future for 2019,2020,2021
		}
		else
			delete lineData[key];
	});
...
var color = d3.scaleOrdinal(d3.schemeCategory10);
	...
	var x = d3.scaleTime().domain(d3.extent(data, function(d) { return d3.timeParse("%Y-%m-%d")(d["EventDate"]); })).range([0, width-10]),
    x2 = d3.scaleTime().domain(d3.extent(data, function(d) { return d3.timeParse("%Y-%m-%d")(d["EventDate"]); })).range([0, width-10]),
    y = d3.scaleLinear().domain([0, d3.max(data, function(d) { return +d.Injuries; }) + 5]).range([height, 0]),
    y2 = d3.scaleLinear().domain([0, d3.max(data, function(d) { return +d.Injuries; }) + 5]).range([height2, 0]);
...
var line2 = d3.line()
    .defined(function(d) { return !isNaN(d.temperature); })
    .x(function(d) {return x2(d["EventDate"]); })
...
svg.append("defs").append("clipPath")
	...
	color.domain(d3.keys(data[0]).filter(function(key) { return key !== "EventDate"; }));
...
var context = svg.append("g")
  .attr("transform", "translate(" + margin2.left + "," + margin2.top + ")");
    ...
    x.domain(d3.extent(data, function(d) { return d["EventDate"]; }));
    ...
    mouseG.append('svg:rect') 
      ...
      var bisect = d3.bisector(function(d) {  return d["EventDate"]; }).right;
	  ...
	  while (true){
            ...
            d3.select(this).select('text')
              .text(y.invert(pos.y).toFixed(2)).attr("style","font-weight:700;color:black");
...
var svg_bubble = d3.select("#bubblechart")
  .append("svg")
...
fetch("http://localhost:5000/getClusterData",{method:"GET"}).then(response =>{
  ...
  var tooltip_bubble = d3.select("#bubblechart")
    .append("div")
      .style("opacity", 0)
      .attr("class", "tooltip")
  ...
  var highlight = function(d){
    ...
    d3.selectAll(".bubbles").style("opacity", .05);			// reduce opacity of all groups
  ...
  svg_bubble.append('g')
    ...
    .enter()
    .append("circle")
      .attr("class", function(d) { return "bubbles c" + d.labels })
      .attr("cx", function (d) { return x(d.Injuries); } )
      .attr("cy", function (d) { return y(d.TotalUninjured); } )
      .attr("r", function (d) { return z(d.TotalFatalInjuries); } )
      .style("fill", function (d) { return myColor("c"+d.labels); } )
    ...
    svg_bubble.selectAll("myrect")
      .data(allgroups)
      .enter()
      .append("circle")
        .attr("cx", legend_x)
        .attr("cy", function(d,i){ return legend_y + i*(size+5)})
        .attr("r", 7)
    ...
    svg_bubble.selectAll("mylabels")
      .data(allgroups)
      .enter()
      .append("text")
        .attr("x", legend_x + size*.8)
        .attr("y", function(d,i){ return legend_y -10 + i * (size + 5) + (size/2)})
        .style("fill", function(d){ return myColor(d)})
